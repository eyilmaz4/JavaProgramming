...
public class HMmultiplicationIndex {
    public static void main(String[] args) {
        int [] arr={1,2,3,5,2,4};
        multiplyAdjacent(arr);
    }
    public static void multiplyAdjacent(int [] arr){

        HashMap<Integer, Integer>map=new HashMap<>();
        for(int i=0; i<arr.length-1; i++){
            map.put(i, arr[i]*arr[i+1]);
        ...
        System.out.println("index 1: "+index+"index 2: "+(index+1)+"\t=\t"+max);
